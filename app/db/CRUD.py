...
class BaseCRUD:
    # Sellers
    @classmethod
    async def add_product_seller(
        cls, param: SProducts, user_id: int, categories: str, photos: list[UploadFile]
    ) -> int:
        async with async_session() as session:
            ...
            if photos is not None:
                for i, photo in enumerate(photos):
                    file = Photos(
                        product_id=product_id, photo=f"product{product_id}_{i + 1}.jpg"
                    )
                    session.add(file)
                    await session.commit()
            return product_id
    ...
    async def add_parameters(cls, param: dict, user_id: int, product_id: int):
        async with async_session() as session:
            ...
            if param is not None:
                param = [
                    {"name": key, "description": value, "product_id": product_id}
    ...
    async def add_manager(cls, user_id: int, manager_id: int):
        async with async_session() as session:
            company_info = (
                await session.execute(
                    select(Sellers.company_id, Sellers.type_company).filter_by(
                        user_id=user_id
                    )
                )
            ).first()
    ...
    async def get_products_category(cls, category_name: str) -> list[SProductsInfo]:
        async with async_session() as session:
            ...
            if products is None:
                raise HTTPException(
                    status_code=404, detail="Products not found in this category"
                )
            products_model = [
        ...
        async with async_session() as session:
            products = (
                await session.execute(
                    ...
                    .options(
                        selectinload(Products.photo), selectinload(Products.parameter)
                    )
                )
            ).first()
            if products is None:
    ...
    async def get_account_info(cls, user_id: int) -> SAccountInfo:
        async with async_session() as session:
            ...
            user = [
                SAccountInfo.model_validate(result, from_attributes=True)
                for result in user_resp
            ]
            return user[0]
    ...
    async def get_review_by_product_id(cls, param: SProduct) -> list[SReviewInfo]:
        async with async_session() as session:
            ...
            reviews = [
                SReviewInfo.model_validate(result, from_attributes=True)
                for result in resp
            ]
            return reviews
